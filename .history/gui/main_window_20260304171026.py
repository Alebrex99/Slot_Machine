...
class MainWindow(QWidget):
    ...
    def _update_reels(self) -> None:
        ...
        for reel, sym in zip((self.reel1, self.reel2, self.reel3), self.reel_symbols):
            # OLD: scaledToWidth(size) — scales width=size but height = size*(img_h/img_w);
            #      for portrait images the pixmap is taller than `size` → QLabel clips from top.
            # NEW: scaled(size, size, KeepAspectRatio) — image always fits inside size×size box
            #      regardless of aspect ratio: portrait images shrink to fit height, no clipping.
            reel.setPixmap(self.symbols[sym].scaled(size, size, Qt.KeepAspectRatio, Qt.SmoothTransformation))
    ...
    def animate_spin(self):
        ...
        size = self._get_symbol_size()
        # OLD: scaledToWidth(size) — same portrait-clipping bug as in _update_reels
        # NEW: scaled(size, size, KeepAspectRatio) — consistent with _update_reels, no clipping
        self.reel1.setPixmap(random.choice(random_symbols).scaled(size, size, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        self.reel2.setPixmap(random.choice(random_symbols).scaled(size, size, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        self.reel3.setPixmap(random.choice(random_symbols).scaled(size, size, Qt.KeepAspectRatio, Qt.SmoothTransformation))
