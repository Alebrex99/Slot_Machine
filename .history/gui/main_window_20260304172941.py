...
class MainWindow(QWidget):
    ...
    def _get_symbol_size(self) -> int:
        """Fallback size estimate used only during animation frames (reel dims not yet settled).
        Returns the side of the largest square that fits the current window, capped at reel max."""
        w = self.width() 
        h = self.height()
    ...
    def _update_reels(self) -> None:
        ...
        for reel, sym in zip((self.reel1, self.reel2, self.reel3), self.reel_symbols):
            ...
            w = reel.width()  if reel.width()  > 0 else 80
            h = reel.height() if reel.height() > 0 else 80
            reel.setPixmap(
