...
class MainWindow(QWidget):
    def __init__(self, metrics_logger: MetricsLogger):
        Timer Mechanism:
        The spin_timer uses a timeout signal that triggers the animate_spin() method at regular intervals:
        - Interval: spin_speed (80 ms) - controls the frame refresh rate
        - Total frames: roll_frames (50) - number of animation frames
        - Total animation duration: spin_speed * roll_frames = 80ms * 50 = 4000ms (4 seconds)
        - Frame rate: 1000ms / spin_speed = 1000ms / 80ms ≈ 12.5 FPS
        When spin_timer.start(self.spin_speed) is called, the timer begins emitting timeout signals every 80ms.
        Each timeout signal invokes animate_spin(), which updates the reels' displayed symbols and increments 
        current_frame until reaching roll_frames, at which point the animation completes and the timer stops.
        """
    ...
    def show_final_result(self):
        ...
        #TODO: salvare BET negativa in caso di perdita in RESULT
        self.coins += reward
        self.update_coin_label()
        self.validate_bet()
