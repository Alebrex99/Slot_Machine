...
class MainWindow(QWidget):
    def __init__(self, metrics_logger: MetricsLogger):
        ...
        for reel in (self.reel1, self.reel2, self.reel3):
            ...
            # OLD: QSizePolicy.Expanding — sizeHint() = pixmap size → feedback loop: each animate_spin frame
            #      read a larger reel.width() because the layout honoured the growing sizeHint → reels grew
            # NEW: QSizePolicy.Ignored — layout ignores sizeHint entirely; reel size driven only by
            #      stretch factors and window geometry → no feedback loop possible
            reel.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
            reel.setMinimumSize(80, 80)    # prevent collapse to 0 when window is very small
            reel.setMaximumSize(700, 700)  # cap so reels don't exceed intended max
