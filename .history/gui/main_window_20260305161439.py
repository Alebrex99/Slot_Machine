...
class MainWindow(QWidget):
    def __init__(self, metrics_logger: MetricsLogger):
        ...
        self.bet_display.setFixedHeight(100)  # BUG1: prevent vertical compression / content clipping
        ...
        self.bet_up_btn.setFixedHeight(100)  # BUG1: prevent overlap when window shrinks vertically
        ...
        self.bet_down_btn.setFixedHeight(100)  # BUG1: prevent overlap when window shrinks vertically
    ...
    def _compute_reel_size(self) -> int:
        ...
        w = max(self.width(), 1)
        h = max(self.height(), 1)
        available_w = max(80, (w - 80) // 3)   # 80 px total horizontal padding, 3 reels
        available_h = max(80, h - 220)          # ~220 px reserved for top + bottom sections
        return min(min(available_w, available_h), 700)
