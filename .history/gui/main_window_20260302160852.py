...
INITIAL_BUDGET = 100
MIN_BET = 0.10
MAX_BET = 2.0  # clamp any input above this to MAX_BET
BET_STEP = 0.10
TOTAL_SESSION_BETS = 60  # 3 phases of 20 bets each
PHASE_LENGTH = 20
class MainWindow(QWidget):
    def __init__(self, metrics_logger: MetricsLogger):
        ...
        self.initial_budget_after = None  # set once when bet_counter first reaches PHASE_AFTER
    ...
    def on_spin(self):  
        ...
        # Track phase-start budget for AFTER phase (set once on first AFTER bet)
        if self.bet_counter == PHASE_LENGTH * 2 + 1 and self.initial_budget_after is None:
            self.initial_budget_after = budget_before_spin
        self.current_reward, multiplier = calculate_reward(budget_before_spin, self.bet_counter, self.current_bet, self.initial_budget_after)
    ...
    def show_final_result(self):
        ...
        # Fino a questo punto è stato già fatto con lo spin: coins = coins - bet 
        # reward = bet * multiplier
        # quindi facendo coins = coins + reward -> coins = coins - bet + bet*multiplier 
    ...
    def _execute_spin_logic(self) -> None:
        ...
        if self.bet_counter == PHASE_LENGTH * 2 + 1 and self.initial_budget_after is None:
            self.initial_budget_after = budget_before_spin
        reward, multiplier = calculate_reward(budget_before_spin, self.bet_counter, self.current_bet, self.initial_budget_after)
        r1, r2, r3 = spin_reels(reward, multiplier)
        self.coins += reward
    ...
    def testing_statistics(self, researcher) -> None:
        ...
        self.coins = INITIAL_BUDGET
        self.bet_counter = 0
        self.initial_budget_after = None
