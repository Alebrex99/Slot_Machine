...
class MainWindow(QWidget):
    def __init__(self, metrics_logger: MetricsLogger):
        ...
        reels = QHBoxLayout()
        # OLD: reels.setAlignment(Qt.AlignCenter)  — with Ignored policy sizeHint=0, AlignCenter collapses all widgets
        # With Ignored policy, stretch factor on addWidget is the ONLY driver of horizontal sizing.
        # stretch=1 on each reel → 3 equal shares; stretch=1 on margins → centering padding.
        reels.addStretch(1)
        reels.addWidget(self.reel1, 1)   # stretch=1 → equal share of available width
        reels.addWidget(self.reel2, 1)
        reels.addWidget(self.reel3, 1)
        reels.addStretch(1)
        ...
        root = QVBoxLayout()
        root.addLayout(top)
        # OLD: root.addStretch() + root.addLayout(reels) + root.addStretch()
        #      reels had stretch=0 → with Ignored policy got 0 height, squeezed between two stretches
        # NEW: reels layout gets stretch=1 → claims all remaining vertical space between top and bottom
        root.addLayout(reels, 1)
        # OLD:
