...
class MainWindow(QWidget):
    def __init__(self, metrics_logger: MetricsLogger):
        ...
        for reel in (self.reel1, self.reel2, self.reel3):
            ...
            reel.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)  # Allow pixmap to scale freely with the label size
        ...
        reels = QHBoxLayout()
        # OLD: reels.setAlignment(Qt.AlignCenter)  # redundant with addStretch(); caused reels to stack on top
        #      of each other when QSizePolicy.Ignored is used (sizeHint=0 → AlignCenter collapses all to one point)
        reels.addStretch()
