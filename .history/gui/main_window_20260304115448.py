...
class MainWindow(QWidget):
    def __init__(self, metrics_logger: MetricsLogger):
        ...
        # OLD: self.coin_label.setMinimumWidth(200)  # hardcoded, made right column wider than left
        # NEW: no fixed width — the 3-equal-column top layout makes both sides equal naturally
        ...
        for reel in (self.reel1, self.reel2, self.reel3):
            ...
            reel.setMinimumSize(80, 80)  # minimum so the window can shrink freely
            reel.setMaximumSize(1000, 1000)  # arbitrary large max to allow expansion
        ...
        # MUSIC CONTROLS
        # OLD: self.music_btn = QPushButton("Music Off")  # label was inverted (music starts ON)
        # NEW: label reflects actual initial state (music starts playing at startup)
        ...
        #            LAYOUTS
        # ===============================
        # OLD: top used addStretch() spacers between three widgets — watermark was not truly centred
        #      because left and right sections could differ in width.
        # NEW: three equal-weight columns (stretch=1 each) so watermark is always exactly centred.

        # Left column: bet controls (up / display / down), vertically centred
        bet_col = QVBoxLayout()
        bet_col.addStretch()
        bet_col.addWidget(self.bet_up_btn)
        bet_col.addWidget(self.bet_display)
        bet_col.addWidget(self.bet_down_btn)
        bet_col.addStretch()

        # Centre column: watermark, vertically centred
        center_col = QVBoxLayout()
        center_col.addStretch()
        center_col.addWidget(self.watermark)
        center_col.addStretch()

        # Right column: redeem button + coin label, vertically centred, right-aligned inside the column
        right_col = QVBoxLayout()
        right_col.addStretch()
        right_col.addWidget(self.redeem_btn, alignment=Qt.AlignRight)
        right_col.addWidget(self.coin_label, alignment=Qt.AlignRight)
        right_col.addStretch()

        top = QHBoxLayout()
        top.addLayout(bet_col, 1)     # 1/3 width
        top.addLayout(center_col, 1)  # 1/3 width — watermark always centred
        top.addLayout(right_col, 1)   # 1/3 width
        ...
        # OLD: bottom = QHBoxLayout() with [music_btn][stretch][spin_btn][stretch]
        #      The music button consumed left space so spin_btn was not truly centred.
        # NEW: three equal-weight sections — music left | GIOCA centred | empty right mirror.
        bottom_left = QHBoxLayout()
        bottom_left.addWidget(self.music_btn)
        bottom_left.addStretch()

        bottom_center = QHBoxLayout()
        bottom_center.addWidget(self.spin_btn, alignment=Qt.AlignCenter)

        bottom = QHBoxLayout()
        bottom.addLayout(bottom_left, 1)    # 1/3 — music anchored left
        bottom.addLayout(bottom_center, 1)  # 1/3 — GIOCA truly centred
        bottom.addStretch(1)                # 1/3 — empty mirror of left section
    ...
    def _get_symbol_size(self) -> int:
        ...
        # Respect the reel's setMaximumSize(1000, 1000)
        return min(computed_size, 1000)
