...
class MainWindow(QWidget):
    ...
    def _update_reels(self) -> None:
        ...
        for reel, sym in zip((self.reel1, self.reel2, self.reel3), self.reel_symbols):
            # OLD: scaledToWidth(size) — vincolava solo la larghezza; immagini portrait eccedevano in altezza → crop in cima
            # NEW: scaled(size, size, KeepAspectRatio) — inscrive il pixmap in un bounding box size×size; nessuna dimensione eccede
    ...
    def animate_spin(self):
        ...
        size = self._get_symbol_size()
        # OLD: scaledToWidth(size) — stesso bug di _update_reels: crop in cima per immagini non quadrate
        # NEW: scaled(size, size, KeepAspectRatio) — coerente con _update_reels
