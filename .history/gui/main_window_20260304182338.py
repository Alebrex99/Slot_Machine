...
class MainWindow(QWidget):
    def __init__(self, metrics_logger: MetricsLogger):
        ...
        reels.addStretch()
        reels.addWidget(self.reel1, 2)
        reels.addWidget(self.reel2, 2)
        reels.addWidget(self.reel3, 2)
        reels.addStretch(1)
    ...
    def _get_reel_size(self) -> int:
        """Calculate the square side for each reel based on actual widget geometry.

        Returns:
            Side length in pixels, clamped to [80, 700].
        """
        # OLD: used self.width()/self.height() estimates that could be stale
        # NEW: uses reel's own allocated geometry — stable because Ignored policy
        #      means layout doesn't depend on this value (no feedback loop)
        reel = self.reel1
        side = min(reel.width(), reel.height())
        return max(80, min(side, 700))
    ...
    def _update_reels(self) -> None:
        ...
        for reel, sym_name in zip(
            (self.reel1, self.reel2, self.reel3),
            self.reel_symbols,
        ):
            if sym_name and sym_name in self.symbols:
                reel.setPixmap(
                    self.symbols[sym_name].scaled(
    ...
    def on_spin(self):  
        ...
        self._anim_size = self._get_reel_size()
