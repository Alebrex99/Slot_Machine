...
class MainWindow(QWidget):
    def __init__(self, metrics_logger: MetricsLogger):
        ...
        for reel in (self.reel1, self.reel2, self.reel3):
            ...
            # Ignored policy: sizeHint (= pixmap size) is completely ignored by the layout,
            # preventing the feedback loop where setPixmap → bigger sizeHint → layout grows → bigger pixmap.
            # Space is allocated solely via stretch factors in the reels QHBoxLayout.
            reel.setSizePolicy(QSizePolicy.Ignored, QSizePolicy.Ignored)
            reel.setMinimumSize(80, 80)    # small minimum so 3 reels fit in the 600px min window width
            reel.setMaximumSize(700, 700)  # cap so reels don't get excessively large
    ...
    def animate_spin(self):
        ...
        for reel in (self.reel1, self.reel2, self.reel3):
            w = reel.width()  if reel.width()  > 0 else self._get_symbol_size()
            h = reel.height() if reel.height() > 0 else self._get_symbol_size()
            reel.setPixmap(
