...
class MainWindow(QWidget):
    def __init__(self, metrics_logger: MetricsLogger):
        ...
        for reel in (self.reel1, self.reel2, self.reel3):
            ...
            # La dimensione del reel è controllata SOLO da setFixedSize().
            # setFixedSize impedisce al layout di ridimensionare il widget in base al pixmap
            # (nessun feedback loop possibile). resizeEvent() aggiorna setFixedSize() al resize della finestra.
            reel.setFixedSize(400, 400)  # dimensione iniziale; verrà ricalcolata in resizeEvent
        ...
        top = QHBoxLayout()
        top.setAlignment(Qt.AlignTop)                        # NEW: top-align so both columns start flush
    ...
    # ------------------------------------------------------------------
    # RESPONSIVE REEL HELPERS
    # _get_reel_size() computes the optimal square size for each reel from the window geometry.
    # _update_reels() redraws all three reels at that size.
    # resizeEvent() recalculates reel fixed size + redraws pixmaps on window resize.
    # ------------------------------------------------------------------

    def _get_reel_size(self) -> int:
        """Compute the side length of each reel square from the current window size.

        Divides available horizontal space by 3 (one per reel), reserves ~280 px
        vertically for top controls + bottom bar, and takes the minimum so the
        square fits in both dimensions. Clamped to [80, 700].
        """
        w = self.width()  if self.width()  > 0 else 600
        h = self.height() if self.height() > 0 else 400
        available_w = (w - 40) // 3   # horizontal share per reel (minus margins)
        available_h = h - 280          # vertical space after top/bottom bars
        size = min(available_w, available_h)
        return max(80, min(size, 700))

    def _update_reels(self) -> None:
        """Rescale and redisplay the current reel symbols to fit the reel's fixed size.

        Reads reel.width()/height() which are stable (set by setFixedSize in resizeEvent),
    ...
    def resizeEvent(self, event) -> None:  # noqa: N802
        """On window resize: recalculate reel fixed size, then redraw pixmaps."""
        super().resizeEvent(event)
        size = self._get_reel_size()
        for reel in (self.reel1, self.reel2, self.reel3):
    ...
    def animate_spin(self):
        ...
        # Legge reel.width()/height() che sono STABILI (setFixedSize — non cambiano per colpa del pixmap).
        # Si aggiornano solo se l'utente ridimensiona la finestra (resizeEvent → setFixedSize).
        for reel in (self.reel1, self.reel2, self.reel3):
            reel.setPixmap(
                random.choice(random_symbols).scaled(
                    reel.width(), reel.height(), Qt.KeepAspectRatio, Qt.SmoothTransformation
                )
            )
