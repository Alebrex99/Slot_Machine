...
class MainWindow(QWidget):
    def __init__(self, metrics_logger: MetricsLogger):
        ...
        # NEW: Bottom Layout
        # music (left, fixed 300 px) | stretch | GIOCA (centre) | stretch | spacing (right, 300 px)
        bottom = QHBoxLayout()
        bottom.addWidget(self.music_btn)                             # fixed 300 px (setFixedWidth)
        bottom.addStretch(1)
        bottom.addWidget(self.spin_btn)
        bottom.addStretch(1)
        bottom.addSpacing(self.music_btn.sizeHint().width())         # NEW: mirrors music_btn → GIOCA truly centred
        # OLD: bottom.addStretch()  # nothing on right → spin_btn was shifted left
