...
class MainWindow(QWidget):
    def __init__(self, metrics_logger: MetricsLogger):
        ...
        # OLD: root.addWidget(self.spin_btn, alignment=Qt.AlignCenter)  # spin alone on its own row
        # OLD: music_layout = QHBoxLayout()                              # music added BELOW spin in a separate row
        # OLD: music_layout.addWidget(self.music_btn)
        # OLD: root.addLayout(music_layout)
        # NEW: single bottom row — music (left) | stretch | GIOCA (centre) | stretch
        bottom = QHBoxLayout()
    ...
    def _get_symbol_size(self) -> int:
        """Compute reel pixmap size that fits the current window dimensions."""
        ...
        return min(available_w, available_h)
