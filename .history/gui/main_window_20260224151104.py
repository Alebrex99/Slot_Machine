...
class MainWindow(QWidget):
    def __init__(self, metrics_logger: MetricsLogger):
        The spin animation timer operates as follows:
        A QTimer is configured to emit a timeout signal at regular intervals (spin_speed = 80ms).
        Each timeout triggers the animate_spin callback function, which updates the reel display
        for the current animation frame. The animation runs for a fixed number of frames (roll_frames = 50),
        resulting in a total animation duration of spin_speed × roll_frames (80ms × 50 = 4000ms = 4 seconds).
        This produces an approximate frame rate of 1000ms / spin_speed = 12.5 FPS.
        Args:
            metrics_logger (MetricsLogger): Logger instance for tracking game metrics and events.
        """
    ...
    def show_final_result(self):
        ...
        #TODO: salvare BET negativa in caso di perdita in RESULT             bet=self.current_bet,
