...
def calculate_reward(budget_before_spin, current_bet_counter, current_bet):
    ...
    if current_bet_counter in PHASES["PHASE_BEFORE"]:
        ...
        if win:
            ...
            reward, multiplier = evaluate_win(initial_budget_before, budget_before_spin, current_bet)
    ...
    if current_bet_counter in PHASES["PHASE_DURING"]:
        if condition == "EQUAL":
            ...
            if win:
                reward, multiplier = _flat_phase_reward(initial_budget_during, budget_before_spin, current_bet)
                print(f"VITTORIA EQUAL! Bet n° {current_bet_counter} ({multiplier}x) -> reward: {reward}")
                return reward, multiplier
            return 0, None
        elif condition == "WIN":
    ...
    if current_bet_counter in PHASES["PHASE_AFTER"]:
        ...
        if win:
            ...
            reward, multiplier = evaluate_win(initial_budget_after, budget_before_spin, current_bet)
            print(f"VITTORIA! Bet n° {current_bet_counter} (moltiplicatore ideale: {multiplier}x) -> current_bet: {current_bet}, expected_reward: {expected_reward}, reward: {reward}")
            return (reward, multiplier)
        return (0, None)


def evaluate_win(initial_budget_phase, budget_before_spin, current_bet):
    """
