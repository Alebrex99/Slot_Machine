...
CONDITION = "EQUAL"  # Default condition, can be updated by RemoteResearcher
...
def update_condition(condition: str) -> None:
	...
	2) CONDIZIONE WIN: lo user rimane nell'intorno del 100 all'inizio (prime 20 puntate), da 20-esima a 40-esima vinci , poi da 40 a 60 torni piatto intorno a 100 (vincendo e perdendo in modo da rimanere intorno a 100)
	3) CONDIZONE LOSE : lo user rimane intorno a 100, sta piatto, poi PERDI e ti ristabilizzi d un valore specifico (in base a quanto hai perso dopo N tentativi)
    '''
