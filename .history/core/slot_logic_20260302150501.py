...
def calculate_reward(budget_before_spin, current_bet_counter, current_bet):
    ...
    if current_bet_counter in PHASES["PHASE_BEFORE"]:
        ...
        if win:
            ...
            # calcolo reward attesa finale di questa fase: vince tutto ciò che ha perso fino ad ora
            expected_reward = initial_budget_before - budget_before_spin + current_bet
            ideal_multiplier = round(expected_reward / current_bet, 0)
            
            if ideal_multiplier not in REWARD_TABLE_MUL:
                print(f"Warning: ideal_multiplier {ideal_multiplier} not in REWARD_TABLE_MUL")
                # Se non trovo il moltiplicatore ideale, calcolo real_multiplier, ovvero prendo quello subito sotto
                # se però il moltiplicatore ideale subito sotto non c'è, prendo quello subito sopra
                real_multiplier = max([m for m in REWARD_TABLE_MUL.keys() if m <= ideal_multiplier], default=min(REWARD_TABLE_MUL.keys()))
                symbol, occurrences = REWARD_TABLE_MUL[real_multiplier]
                print(f"VITTORIA! (moltiplicatore reale: {real_multiplier}x) -> simbolo: {symbol}, occorrenze: {occurrences}")
            # IDEAL_MULTIPLIER
            if ideal_multiplier in REWARD_TABLE_MUL:
                symbol, occurrences = REWARD_TABLE_MUL[ideal_multiplier]
                print(f"VITTORIA! (moltiplicatore ideale: {ideal_multiplier}x) -> simbolo: {symbol}, occorrenze: {occurrences}")
            ...
            # diversamente dall'excell: una volta trovato simbolo e occorrenze, bisogna ritornare la reward effettiva (e visualizzata)
            # ovvero il valore totale inclusa la bet (che ho già tolto dai coins non appena si clicca spin)
            reward = current_bet * real_multiplier
            print(f"Reward effettiva (inclusa la bet): {reward}")
            return reward
        else: 
            return 0
...
def spin_reels(reward):
    ...
    # WIN: usa il moltiplicatore per determinare il simbolo e le occorrenze
    if reward > 0:   
        # trova il moltiplicatore corrispondente al reward
        multiplier = reward / current_bet if current_bet > 0 else 1
        # trova il moltiplicatore più vicino nella tabella
        closest_multiplier = min(REWARD_TABLE_MUL.keys(), key=lambda x: abs(x - multiplier))
        symbol, occurrences = REWARD_TABLE_MUL[closest_multiplier]
        # restituisci una combinazione con il simbolo e le occorrenze corrispondenti
        result = [symbol] * occurrences
        # riempi i simboli rimanenti con simboli diversi
        available_symbols = [s for s in SYMBOLS if s != symbol]
        while len(result) < 3:
            result.append(random.choice(available_symbols))
        return tuple(result)
