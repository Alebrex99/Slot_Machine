...
def lose_increase(budget_before_spin, current_bet_counter, current_bet):
    ...
    if lose_difference > 0: # lose_value > expected_lose_value
        ...
        reward = round(current_bet * multiplier, 2)
        print(f"CHECKPOINT CONTENTINO! Bet n° {current_bet_counter},  lose_value: {lose_value}, expected_lose_value: {expected_lose_value}, lose_difference: {lose_difference}, multiplier: {multiplier}x, reward: {reward}")
        return reward, multiplier
    # altrimenti non hai perso abbastanza
        ...
        reward = round(current_bet * multiplier, 2)
        print(f"CHECKPOINT MINIMO! Bet n° {current_bet_counter},  lose_value: {lose_value}, expected_lose_value: {expected_lose_value}, lose_difference: {lose_difference}, multiplier: {multiplier}x, reward: {reward}")
        return reward, multiplier

# TODO: controllare dall'excell perchè, per il calcolo reward si faccia: IF (multiplier*bet)<0 -> 0 , ELSE (multiplier*bet)
