...
class MetricsLogger:
    ...
    def enable_metrics(self, condition: str) -> None:
        ...
        """
        # Accept short-form codes 'E','W','L' or full-form in VALID_CONDITIONS
        short_map = {"E": "EQUAL", "W": "WIN", "L": "LOSE"}
        if condition in short_map:
            condition_full = short_map[condition]
        else:
        ...
        if condition_full not in VALID_CONDITIONS:
            raise ValueError(
                f"Invalid condition '{condition}'. "
                f"Allowed values: {list(short_map.keys())} or {VALID_CONDITIONS}"
            )

        self._current_condition = condition_full
    ...
    def log_bet(self, bet_number: int, bet: float, condition: str, result_gain: float, current_coin: float) -> None:
        ...
        # Normalize short-form condition if provided
        short_map = {"E": "EQUAL", "W": "WIN", "L": "LOSE"}
        cond = short_map.get(condition, condition)
