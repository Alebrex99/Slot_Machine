...
class RemoteResearcher:
    ...
    def set_input_data(self) -> None:
        ...
        # TEST + CONDITION
        # l'utente può inserire "TEST EQUAL" o "TEST WIN" o "TEST LOSE" per attivare la modalità di test automatico con la condition desiderata
        # lettura dell'intera stringa inserita
        if self._input_data.upper().startswith("TEST "):
            parts = self._input_data.upper().split()
            if len(parts) != 2:
