...
def spin_reels(reward, multiplier):  # multiplier già calcolato da calculate_multiplier
    if reward == 0:
        symbols = random.sample(SYMBOLS, 3)
        return tuple(symbols)
    
    if reward > 0:
        symbol, occurrences = REWARD_TABLE_MUL[multiplier]  # usa direttamente multiplier, già validato
# ...existing code...
