...
class RemoteResearcher:
    ...
    def set_condition(self, condition: str) -> None:
        ...
        if condition not in VALID_CONDITIONS:
            # richiamo set_input_data per chiedere nuovamente l'input al ricercatore, finché non fornisce un valore valido
            print(f"[RemoteResearcher] condition '{condition}' non valida. Valori consentiti: {VALID_CONDITIONS}")
            self.set_input_data()  # Richiama set_input_data per richiedere un input valido
            return  # Non proseguire con l'aggiornamento della condition, ciclo di richiesta
