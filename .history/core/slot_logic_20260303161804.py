...
def lose_increase(budget_before_spin, current_bet_counter, current_bet):
    # FIX: spec formula is LOSE_VALUE = INITIAL_BUDGET_DURING - BUDGET_BEFORE_SPIN + BET
    #      and EXPECTED_LOSE_VALUE = PCT * INITIAL_BUDGET_DURING + BET
    #      both terms include + current_bet so it cancels in DIFFERENCE, but must be present
    #      for correct absolute-value comparisons and multiplier calculation.
    
    # WRONG (old): lose_value = initial_budget_during - budget_before_spin
    # WRONG (old): expected_lose_value = round(expected_percentage_decrease * initial_budget_during, 2)
    lose_value = round(initial_budget_during - budget_before_spin + current_bet, 2)  # FIX: + current_bet added
    expected_percentage_decrease = EXPECTED_PERCENTAGE_DECREASES[current_bet_counter]
    expected_lose_value = round(expected_percentage_decrease * initial_budget_during + current_bet, 2)  # FIX: + current_bet added
