...
def lose_increase(budget_before_spin, current_bet_counter, current_bet):
    lose_value = initial_budget_during - budget_before_spin + current_bet # calcolo la perdita effettiva cumulata fino ad ora, includendo la bet corrente
    expected_percentage_decrease = EXPECTED_PERCENTAGE_DECREASES[current_bet_counter]
    expected_lose_value = round(expected_percentage_decrease * initial_budget_during + current_bet, 2) # calcolo la perdita attesa per questa bet
    lose_difference = lose_value - expected_lose_value # calcolo la differenza tra perdita attesa e perdita effettiva
    pass
