...
def calculate_reward(budget_before_spin, current_bet_counter, current_bet):
    ...
    if current_bet_counter in PHASES["PHASE_DURING"]:
        ...
        # WIN
            ...
            if win:
                reward, multiplier = win_increase(initial_budget_during, current_bet_counter, current_bet)
                return reward, multiplier
            return 0, None
        # LOSE
        elif condition == "LOSE":
...
# PER DURING_WIN
def win_increase(initial_budget_phase, current_bet_counter, current_bet):
