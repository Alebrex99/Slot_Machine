...
def calculate_reward(budget_before_spin, current_bet_counter, current_bet):
    ...
    if current_bet_counter in PHASES["PHASE_BEFORE"]:
        ...
        if win:
            ...
            #Sposto la parte del multiplier, che si ripeterà, in spin_reels
            if ideal_multiplier in REWARD_TABLE_MUL:
                reward = current_bet * ideal_multiplier
                print(f"VITTORIA! (moltiplicatore ideale: {ideal_multiplier}x) -> reward: {reward}")
                ...
                print(f"VITTORIA! (moltiplicatore reale: {real_multiplier}x) -> reward: {reward}")
