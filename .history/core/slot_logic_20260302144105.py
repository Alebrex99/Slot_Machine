...
def calculate_reward(budget_before_spin, current_bet):
    ...
    if current_bet_counter in PHASES["PHASE_BEFORE"]:
        ...
        if win:
            ...
            # diversamente dall'excell: una volta trovato simbolo e occorrenze, bisogna ritornare la reward effettiva (e visualizzata) da mostrare all'utente, che è data da bet * moltiplicatore (reward visualizzata) ma anche da bet * moltiplicatore - bet (reward reale, che si aggiunge alle monete dell'utente):
            # ovvero il valore totale inclusa la bet (che ho già tolto dai coins non appena si clicca spin)
            reward = current_bet * real_multiplier # la reward effettiva è data dalla puntata moltiplicata per il moltiplicatore reale
