...
def win_increase(current_bet_counter, current_bet):
    ...
    multiplier = calculate_multiplier(expected_reward, current_bet)
    reward = round(current_bet * multiplier, 2)
    print(f"VITTORIA! Bet n° {current_bet_counter}, expected_reward: {expected_reward}, expected_%_increase: {expected_percentage_increase}, moltiplicatore: {multiplier}x, reward: {reward}")
    return reward, multiplier
