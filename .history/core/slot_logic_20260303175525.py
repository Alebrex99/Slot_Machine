...
def calculate_reward(budget_before_spin, current_bet_counter, current_bet):
    ...
    if current_bet_counter in PHASES["PHASE_DURING"]:
        ...
        print(f"FASE DURING {current_bet_counter}: initial budget: {initial_budget_during:.2f}, condizione {condition}, bet corrente: {current_bet}, aumento (WIN), diminuzione (LOSE) o stabili (EQUAL)")
        # EQUAL
        if condition == "EQUAL":
