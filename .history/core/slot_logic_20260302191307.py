...
def calculate_reward(budget_before_spin, current_bet_counter, current_bet):
    ...
    if current_bet_counter in PHASES["PHASE_DURING"]:
        ...
        # WIN
            ...
            if win:
                reward, multiplier = win_increase(initial_budget_during, budget_before_spin, current_bet_counter, current_bet)
                return reward, multiplier
            return 0, None
        # LOSE
        elif condition == "LOSE":
...
# PER DURING_WIN
def win_increase(initial_budget_phase, budget_before_spin, current_bet_counter, current_bet):
    expected_percentage_increase = EXPECTED_PERCENTAGE_INCRESES.get(current_bet_counter, 0)
    expected_reward = round(current_bet * expected_percentage_increase, 2)
    return expected_reward

def lose_increase(initial_budget_phase, budget_before_spin, current_bet_counter, current_bet):
    pass
