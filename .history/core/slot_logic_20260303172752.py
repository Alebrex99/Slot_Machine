...
def calculate_reward(budget_before_spin, current_bet_counter, current_bet):
    ...
    if current_bet_counter in PHASES["PHASE_BEFORE"]:
        ...
        print(f"FASE BEFORE {current_bet_counter}, initial budget: {initial_budget_before:.2f}, bet corrente: {current_bet}, rimanere nell'intorno di INITIAL_BUDGET")
    ...
    if current_bet_counter in PHASES["PHASE_DURING"]:
        ...
        print(f"FASE DURING {current_bet_counter}: initial budget: {initial_budget_during:.2f}, condizione {condition}, bet corrente: {current_bet}, aumento (WIN), diminuzione (LOSE) o rimanere stabili (EQUAL)")
        # EQUAL
        if condition == "EQUAL":
...
def win_increase(current_bet_counter, current_bet):
    ...
    # default 0 if bet not in map: expected_reward = current_bet → multiplier = 1 (minimal win)
...
def lose_increase(budget_before_spin, current_bet_counter, current_bet):
    ...
    lose_value = initial_budget_during - budget_before_spin # calcolo la perdita effettiva cumulata fino ad ora, includendo la bet corrente
    # default 0 if bet not in map: expected_lose_value = 0 → lose_difference = full actual loss → consolation win
    expected_percentage_decrease = EXPECTED_PERCENTAGE_DECREASES.get(current_bet_counter, 0)
