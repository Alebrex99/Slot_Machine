...
# updated: import from constants.py, or repeat constants to avoid circular imports
...
def lose_increase(budget_before_spin, current_bet_counter, current_bet):
    ...
    if lose_difference > 0: # lose_value > expected_lose_value
        ...
        reward = round(current_bet * multiplier, 2)
        print(f"CHECKPOINT CONTENTINO! Bet n° {current_bet_counter}, lose_value: {lose_value}, expected_lose_value: {expected_lose_value}, lose_difference: {lose_difference}, multiplier: {multiplier}x, gain reale: {round(reward - current_bet, 2)}, reward: {reward}")
        return reward, multiplier
    # altrimenti non hai perso abbastanza
        ...
        reward = round(current_bet * multiplier, 2)
        print(f"CHECKPOINT MINIMO! Bet n° {current_bet_counter}, lose_value: {lose_value}, expected_lose_value: {expected_lose_value}, lose_difference: {lose_difference}, multiplier: {multiplier}x, gain reale: {round(reward - current_bet, 2)}, reward: {reward}")
        return reward, multiplier
