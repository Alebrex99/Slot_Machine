...
class MetricsLogger:
    ...
    def log_remote_message(self, message: str) -> None:
        """Logs a MEX event received from the researcher application.

        Args:
            message: Arbitrary text message from the researcher.
        """
        # FIX: was event_type="MESSAGE" which is not in the valid event schema.
        # Valid types: SESSION_START, START_METRICS, BET, MEX, SESSION_END.
        # old: self._log(event_type="MESSAGE", message=message)
        self._log(event_type="MEX", message=message)

    def log_remote_change_condition(self, new_condition: str) -> None:
        """Logs a condition change triggered remotely."""
        # FIX: method was missing → remote_researcher.remote_change_condition() would crash.
        self._log(event_type="MEX", message=f"CHANGE_CONDITION={new_condition}")

    def log_remote_charge_coin(self, amount: float, coin_after: float) -> None:
        """Logs a coin charge triggered remotely."""
        # FIX: method was missing → remote_researcher.remote_charge_coin() would crash.
        self._log(event_type="MEX", coin=coin_after, message=f"CHARGE_COIN added={amount:.2f}")
