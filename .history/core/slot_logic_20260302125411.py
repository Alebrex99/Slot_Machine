    ...
    if current_bet_counter in PHASES["PHASE_BEFORE"]:
        ...
        if win:
            ...
            if ideal_multiplier in REWARD_TABLE_MUL:
                ...
                print(f"VITTORIA! REWARD={reward} (moltiplicatore ideale: {ideal_multiplier}x) -> simbolo: {symbol}, occorrenze: {occurrences}")
                ...
                print(f"VITTORIA! REWARD={reward} (moltiplicatore reale: {real_multiplier}x) -> simbolo: {symbol}, occorrenze: {occurrences}")
            
            # Una volta trovato simbolo e occorrenze, bisogna ritornare la reward effettiva
            reward = current_bet * real_multiplier - current_bet # la reward effettiva è data dalla puntata moltiplicata per il moltiplicatore reale, più la puntata stessa che viene restituita in caso di vittoria
            return reward
        else: 
            return 0
