...
def calculate_reward(budget_before_spin, current_bet_counter, current_bet):
    ''' 
        ...
        - verifico se la bet corrisponde ad una vincita/perdita di quella fase
            - se vittoria: 
              calcolo la formula per determinare la reward (sarà poi opera di spin_reels ad usarla per capire il simbolo e i moltiplicatori)
              calcolo l'initial budget della fase (serve solo se hai vinto) e lo salvo in una variabile globale: initial_budget_before, initial_budget_during, initial_budget_after
            - altrimenti: pass
    ...
    if current_bet_counter in PHASES["PHASE_BEFORE"]:
        print(f"FASE BEFORE {current_bet_counter}: rimanere nell'intorno di INITIAL_BUDGET")
        ...
        if win:
            ...
            expected_reward = round(initial_budget_before - budget_before_spin + current_bet, 2)
            multiplier = calculate_multiplier(expected_reward, current_bet)
            reward = round(current_bet * multiplier, 2)
            print(f"VITTORIA! Bet n° {current_bet_counter} (moltiplicatore ideale: {multiplier}x) -> current_bet: {current_bet}, expected_reward: {expected_reward}, reward: {reward}")
    ...
    # FASE AFTER (unisco alla BEFORE)
    '''if current_bet_counter in PHASES["PHASE_AFTER"]:
        # uguale alla fase before: arrivo alla 41 inclusa
        print(f"FASE AFTER {current_bet_counter}: rimanere nell'intorno di INITIAL_BUDGET")
        win = BEFORE_AFTER_PHASE[current_bet_counter - 40]  # usa la stessa mappa della fase before, ma con indice corretto (1-20)
        if win:
            initial_budget_after = budget_before_spin # budget inziale è quello ottenuto dopo la fase during
            expected_reward = round(initial_budget_after - budget_before_spin + current_bet, 2)
            multiplier = calculate_multiplier(expected_reward, current_bet)
            reward = round(current_bet * multiplier, 2)
            print(f"VITTORIA! Bet n° {current_bet_counter} (moltiplicatore ideale: {multiplier}x) -> current_bet: {current_bet}, expected_reward: {expected_reward}, reward: {reward}")
            return (reward, multiplier)
...
def _calculate_flat_phase_reward(budget_before_spin, current_bet_counter, current_bet, phase_initial_budget, phase_name):
    """
    Logica comune a PHASE_BEFORE e PHASE_AFTER:
    il giocatore rimane nell'intorno del budget iniziale della fase.
    
    Args:
        budget_before_spin: budget prima della bet corrente
        current_bet_counter: numero della bet corrente (1-20 per BEFORE, 41-60 per AFTER)
        current_bet: valore della bet corrente
        phase_initial_budget: budget iniziale della fase (INITIAL_BUDGET per BEFORE, budget fine DURING per AFTER)
        phase_name: nome della fase per il logging
    
    Returns:
        tuple (reward, multiplier) — reward=0 e multiplier=None se perdita
    """
    # Normalizza il bet_counter per usare la stessa mappa BEFORE_AFTER_PHASE (chiavi 1-20)
    normalized_counter = current_bet_counter if current_bet_counter <= PHASE_LENGTH else current_bet_counter - 2 * PHASE_LENGTH

    print(f"FASE {phase_name} {current_bet_counter}: rimanere nell'intorno di {phase_initial_budget}")
    win = BEFORE_AFTER_PHASE[normalized_counter]

    if win:
        expected_reward = round(phase_initial_budget - budget_before_spin + current_bet, 2)
        multiplier = calculate_multiplier(expected_reward, current_bet)
        reward = round(current_bet * multiplier, 2)
        print(f"VITTORIA! Bet n° {current_bet_counter} (moltiplicatore ideale: {multiplier}x) -> current_bet: {current_bet}, expected_reward: {expected_reward}, reward: {reward}")
        return (reward, multiplier)
...
def calculate_reward(budget_before_spin, current_bet_counter, current_bet):
    ...
    # FASE BEFORE (bet 1-20)
    if current_bet_counter in PHASES["PHASE_BEFORE"]:
        initial_budget_before = INITIAL_BUDGET
        return _calculate_flat_phase_reward(
            budget_before_spin, current_bet_counter, current_bet,
            phase_initial_budget=initial_budget_before,
            phase_name="BEFORE"
        )

    # FASE DURING (bet 21-40)
    if current_bet_counter in PHASES["PHASE_DURING"]:
        if condition == "EQUAL":
            outcome = DURING_PHASE_EQUAL[current_bet_counter]
        elif condition == "WIN":
            ...
            raise ValueError(f"Invalid condition: {condition}")
        # TODO: completare logica DURING (reward WIN/LOSE/EQUAL)

    # FASE AFTER (bet 41-60)
    if current_bet_counter in PHASES["PHASE_AFTER"]:
        # budget iniziale della fase AFTER = budget alla fine della fase DURING
        initial_budget_after = budget_before_spin if not hasattr(calculate_reward, '_after_budget_set') else initial_budget_after
        return _calculate_flat_phase_reward(
            budget_before_spin, current_bet_counter, current_bet,
            phase_initial_budget=initial_budget_after,
            phase_name="AFTER"
        )
...
def calculate_multiplier(expected_reward, current_bet):
    ...
    if ideal_multiplier in REWARD_TABLE_MUL:
        ...
        real_multiplier = max([m for m in REWARD_TABLE_MUL.keys() if m <= ideal_multiplier], default=min(REWARD_TABLE_MUL.keys()))
