...
def calculate_reward(budget_before_spin, current_bet_counter, current_bet):
    ...
    if current_bet_counter in PHASES["PHASE_BEFORE"]:
        ...
        if win:
            ...
            reward, multiplier = loss_recover(initial_budget_before, budget_before_spin, current_bet)
    ...
    if current_bet_counter in PHASES["PHASE_DURING"]:
        if initial_budget_during is None:
            initial_budget_during = budget_before_spin # il budget iniziale della fase during è quello ottenuto alla fine della fase before, che può essere >, < o = a INITIAL_BUDGET in base a come è andata la fase before
        if condition == "EQUAL":
            win = DURING_PHASE_EQUAL[current_bet_counter]
            if win:
                reward, multiplier = loss_recover(initial_budget_during, budget_before_spin, current_bet)
    ...
    if current_bet_counter in PHASES["PHASE_AFTER"]:
        ...
        if win:
            ...
            reward, multiplier = loss_recover(initial_budget_after, budget_before_spin, current_bet)
            print(f"VITTORIA! Bet n° {current_bet_counter} (moltiplicatore ideale: {multiplier}x) -> current_bet: {current_bet}, expected_reward: {expected_reward}, reward: {reward}")
            return (reward, multiplier)
        return (0, None)


def loss_recover(initial_budget_phase, budget_before_spin, current_bet):
    """
