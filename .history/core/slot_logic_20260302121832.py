    ...
    if current_bet_counter in PHASES["PHASE_BEFORE"]:
        ...
        if win:
            ...
            if ideal_multiplier in REWARD_TABLE_MUL:
                ...
                print(f"VITTORIA! REWARD={reward} (moltiplicatore: {ideal_multiplier}x) -> simbolo: {symbol}, occorrenze: {occurrences}")
            else: # REAL_MULTIPLIER
                # Se non trovo il moltiplicatore ideale, cerco il moltiplicatore più vicino per difetto
                # se il moltiplicatore idea per difetto non c'è, prendo quello per eccesso
                real_multiplier = min(REWARD_TABLE_MUL.keys(), key=lambda x: abs(x - ideal_multiplier))
